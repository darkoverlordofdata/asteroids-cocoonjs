...
GameController =
  ...
  simulateKeyEvent: (eventName, keyCode) ->
    ...
    if typeof jQuery isnt 'undefined'
      ...
      $(@options.canvas).trigger press
      return
    oEvent = document.createEvent('KeyboardEvent')

    # Chromium Hack
    if navigator.userAgent.toLowerCase().indexOf('chrome') isnt -1
  ...
  loadDPad: (side) ->
    ...
    if dpad.up and dpad.left and dpad.down and dpad.right
      options =
        x: posX
        y: posY
  ...
  renderAreas: ->
    ...
    while i < j
      ...
      continue  if typeof area is 'undefined'
      area.draw()
      ...
      while k < l
        touch = @touches[k]
        continue  if typeof touch is 'undefined'
