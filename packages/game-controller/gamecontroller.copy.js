...
( function(exports) {
	...
	var TouchableDirection = ( function( __super ) {
		...
		TouchableDirection.prototype.draw = function() {
			...
			if( ! cached )
			{
                var bitmap = GameController.game.add.bitmapData(this.width + 2 * this.stroke, this.height + 2 * this.stroke);
	...
	var TouchableButton = ( function( __super ) {
		...
		TouchableButton.prototype.draw = function() {
			...
			if( ! cached )
			{
                var bitmap = GameController.game.add.bitmapData(2 * ( this.radius + this.stroke ), 2 * ( this.radius + this.stroke ));
                var ctx = bitmap.context;
				ctx.lineWidth = this.stroke;
	...
	var TouchableJoystick = ( function( __super ) {
		...
		TouchableJoystick.prototype.draw = function() {
			...
			if( ! cached )
			{
                ...
                var hw = 2 * ( this.radius + ( GameController.options.touchRadius ) + this.stroke );

                var bitmap = GameController.game.add.bitmapData(hw, hw);
                var ctx = bitmap.context;
                var gradient = null;
            ...
            var base = GameController.game.add.bitmapData(hw, hw);
	...
	var TouchableCircle = ( function( __super ) {
		...
		function TouchableCircle( options )
		{
            ...
            var bitmap = GameController.game.add.bitmapData(this.radius, this.radius);
